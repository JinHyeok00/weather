...
public class ShortServiceImpl implements ShortService {
    ...
    public Optional<ShortVO> shortService(String baseDate, String baseTime, int nx, int ny) {
        Optional<ShortVO> foundShort = shortDao.find(baseDate, baseTime, nx, ny);
...
//                가져온 정보를 ShortVO에담기
                ...
                shortVO.setResultCode(header.getString("resultCode"));
                shortVO.setResultMsg(header.getString("resultMsg"));
                shortVO.setNumOfRows(1000);
                shortVO.setPageNo(1);
                shortVO.setTotalCount(body.getInt("totalCount"));
                shortVO.setDataType("JSON");
                shortVO.setNx(nx);
                shortVO.setNy(ny);
//                JSON형태의 값을 toString으로 변환에서 저장
                shortVO.setItems(items.toString());
//                DB에 저장해주기
                shortDao.save(shortVO);
//                가져온 정보 리턴하기 (한번에 못찾았을때)
                return shortDao.find(baseDate, baseTime, nx, ny);
            } catch (Exception e) {
            }
        }
